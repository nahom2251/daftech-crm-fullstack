...
public class BrevoApiEmailSender : IEmailSender
{
    ...
    public BrevoApiEmailSender(HttpClient http, IOptions<BrevoApiOptions> options, ILogger<BrevoApiEmailSender> logger)
    {
        ...
        // HttpClient.BaseAddress MUST end with '/' for relative request URIs to
        // append correctly. Without it, "https://api.brevo.com/v3" combined with
        // relative "smtp/email" resolves to "https://api.brevo.com/smtp/email"
        // (the last segment "v3" gets replaced, not extended) — a route that
        // doesn't exist, which is exactly the 404 this fixes.
