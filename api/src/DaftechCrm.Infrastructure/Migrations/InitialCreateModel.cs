// <auto-generated />
//
// Hand-authored (not tool-generated) to match the real EF Core model as of
// the InitialCreate migration, because `dotnet ef migrations add` could not
// be run in the environment this was produced in (no .NET SDK / reachable
// database). Shared between InitialCreate.Designer.cs and
// AppDbContextModelSnapshot.cs so the two can never drift apart — EF only
// requires that BuildTargetModel/BuildModel populate the ModelBuilder
// correctly, not that the code be duplicated inline.
//
// IMPORTANT: if you change any entity configuration in
// Persistence/Configurations/EntityConfigurations.cs, run
//   dotnet ef migrations add <Name> --project src/DaftechCrm.Infrastructure --startup-project src/DaftechCrm.Api
// and let the real tool regenerate everything under Migrations/ from
// scratch — do not hand-edit this file to add a second migration. This
// file's only job is to make InitialCreate accurately reflect the model
// that existed at the time it was written.
using System;
